...
class Trainer(object):
    ...
    #This is our "sampling" function - need to figure out how to obtain counterfactuals from here
    def restore(self, raw_dataloader, shape=None, coef=1e-1, stepsize=1e-1, sampling_steps=50, task='predict'):
        if self.logger is not None:
            tic = time.time()
        ...
        for idx, (x, t_m) in enumerate(raw_dataloader):
            ...
            if sampling_steps == self.model.num_timesteps:
                if task == 'predict-counterfactual':
                    sample = self.ema.ema_model.sample_counterfactual(shape=x.shape, target=x)
                else:
                    sample = self.ema.ema_model.sample_infill(shape=x.shape, target=x * t_m, partial_mask=t_m,
                                                              model_kwargs=model_kwargs)
            else:
                if task == 'predict-counterfactual': #Mask not needed, not multiplying by t_m
                    sample = self.ema.ema_model.fast_sample_counterfactual(shape=x.shape, target=x, dist_grad_fn=1) #Adjust dist_grad_fn accordingly
                else:
