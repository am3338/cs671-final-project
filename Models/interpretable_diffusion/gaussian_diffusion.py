...
class Diffusion_TS(nn.Module):
    ...
    def output(self, x, t, padding_masks=None):
        trend, season = self.model(x, t, padding_masks=padding_masks) #The model decomposes input into trend and season
    ...
    @torch.enable_grad()
    # x_tau denotes the original clean image - need to find a way to retrieve it
    def dist_cond_fn(self, x_tau, z_t, x_t, alpha_t, l1_loss=0, l2_loss=0, l_perc=None): #Check how these are set... l_perc will most likely be used for counterfactuals
        ...
        if isinstance(mv, int):
            ...
            return grads + torch.autograd(mv, x_in)[0] / alpha_t



    #This will be the counterfactual sampling function
    #We essentially need to do the following:
    #1) Implement gradient calculation - DONE
    #2) Implement class_grad_fn (if appropriate) - we are not doing any classification, or class-guided generation, so this is probably not going to be necessary
    #3) Implement dist_grad_fn (if appropriate) - DONE
    #4) The function should include x_tau (original clean image) - DONE
    @torch.no_grad()
    def fast_sample_counterfactual(self, target, shape, clip_denoised=True, dist_grad_fn=None):
        batch, device, total_timesteps, sampling_timesteps, eta = \
            shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.eta
        ...
        #In our case - img -> z_t
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long) #This is probably what we need to pass for the embedding
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, clip_x_start=clip_denoised)
            ...
            # Check if it works when:
            # 1) No gradients are calculated - OK
            # 2) When the L1 gradient is calculated - OK
            # 3) When the L2 gradient is caclulated - OK
            # 4) When both L1 and L2 gradients are calculated - OK
            if dist_grad_fn is not None:
                out = self.p_mean_variance(img, time_cond, clip_denoised=clip_denoised)  # return values: [0]: mean, [1]: variance, [2]: log_variance, [3]: x_start
                grads = 0
                grads = grads + self.dist_cond_fn(x_tau=target, z_t=img, x_t=x_start, alpha_t=alpha, l1_loss=0, l2_loss=0, l_perc=None) #Adjust loss flags as necessary
                out = list(out)
                out[0] = out[0].float() - out[1] * grads
                out = tuple(out)
                img = out[0] + torch.exp(0.5 * out[2])*torch.randn_like(img) #If no gradients are calculated, this is equivalent to the ELSE part
            ### COUNTERFACTUAL - END
            else: #What we are currently doing
                sigma = eta * ((1 - alpha / alpha_next) * (1 - alpha_next) / (1 - alpha)).sqrt()
                c = (1 - alpha_next - sigma ** 2).sqrt()
                noise = torch.randn_like(img)
    ...
    #CHECK DIFFERENCE BETWEEN THIS AND STANDARD FAST SAMPLING
